...
\begin{tikzpicture}[node distance = 0em]
    \tikzstyle{block} = [rectangle, draw=blue, thick, text centered, minimum height = 0em, minimum width = 10em];
    \node [block, fill = blue!00]                               (returned_value_0)  {返回值};
    \node [block, fill = blue!01, below = of returned_value_0]  (parameters_0)      {实在参数};
    \node [block, fill = blue!02, below = of parameters_0]      (link_0)            {可选的访问链};
    \node [block, fill = blue!03, below = of link_0]            (returned_address_0){返回地址};
    \node [block, fill = blue!04, below = of returned_address_0](caller_bp_0)       {调用者bp};
    \node [block, fill = blue!05, below = of caller_bp_0]       (local_data_0)      {局部变量};
    \node [block, fill = blue!06, below = of local_data_0]      (temporaries_0)     {临时区};
    \foreach \i/\j in {1/0, 2/1, 3/2, 4/3, 5/4, 6/5, 7/6, 8/7, 9/8, 10/9, 11/10, 12/11, 13/12, 14/13, 15/14, 16/15}
    {
        \ifnum \i = 5
            \node [block, fill = blue!\i, right = of {returned_value_1}]     (returned_value_\i)  {返回值};
        \else
        \ifnum \i = 9
            \node [block, fill = blue!\i, right = of {returned_value_5}]     (returned_value_\i)  {返回值};
        \else
        \ifnum \i = 13
            \node [block, fill = blue!\i, right = of {returned_value_9}]     (returned_value_\i)  {返回值};
        \else
        \ifnum \i = 17
            \node [block, fill = blue!\i, right = of {returned_value_13}]     (returned_value_\i)  {返回值};
        \else
            \node [block, fill = blue!\i, below = of {temporaries_\j}]     (returned_value_\i)  {返回值};
        \fi
        \fi
        \fi
        \fi
    \node [block, fill = blue!1\i, below = of {returned_value_\i}]  (parameters_\i)      {实在参数};
    \node [block, fill = blue!2\i, below = of {parameters_\i}]      (link_\i)            {可选的访问链};
    \node [block, fill = blue!3\i, below = of {link_\i}]            (returned_address_\i){返回地址};
    \node [block, fill = blue!4\i, below = of {returned_address_\i}](caller_bp_\i)       {调用者bp};
    \draw [->, thick] (caller_bp_\i) to [out = 180, in = 180] (caller_bp_\j);
    \node [block, fill = blue!5\i, below = of {caller_bp_\i}]       (local_data_\i)      {局部变量};
    \node [block, fill = blue!6\i, below = of {local_data_\i}]      (temporaries_\i)     {临时区};
    }
\end{tikzpicture}
\end{figure}
\end{document}
